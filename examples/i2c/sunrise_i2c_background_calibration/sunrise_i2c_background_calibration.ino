 ...
 * @file        sunrise_i2c_background_calibration.ino
 ...
 *              covers operations regarding background calibration.
...
/* If you want to restore factory calibration, set this to 1*/
#define RESTORE_FACTORY_CALIBRATION       (0)

/* Arduino pin which used to connect a button on GND, which will be used to start calibratio process */
...
/* Register Addresses */
const uint8_t ERROR_STATUS_LSB         = 0x01;
const uint8_t CALIBRATION_STATUS       = 0x81;
const uint8_t CALIBRATION_COMMAND_MSB  = 0x82;
const uint8_t CALIBRATION_COMMAND_LSB  = 0x83;
const uint8_t MEASUREMENT_MODE         = 0x95;
const uint8_t METER_CONTROL            = 0xA5;

/* Error statuses */
const uint8_t ERROR_STATUS_LSB_CALIBRATION = 0x80u;

/* Measurement modes */
const uint16_t CONTINUOUS              = 0x0000u;
const uint16_t SINGLE                  = 0x0001u;

/* Calibration status */
const uint8_t CALIBRATION_STATUS_EMPTY               = 0x00u;
const uint8_t CALIBRATION_STATUS_FACTORY_RESTORED    = 0x04u;
const uint8_t CALIBRATION_STATUS_BACKGROUND_COMPLETE = 0x20u;
/* Calibration commands */
const uint8_t CALIBRATION_PREFIX          = 0x7Cu;
const uint8_t CALIBRATION_RESTORE_FACTORY = 0x02u;
const uint8_t CALIBRATION_BACKGROUND      = 0x06u;

/* Error codes */
const int SLAVE_FAILURE                 = -1;
...
/**
 * @brief  Use this function to reset calibration.
 * 
...
int reset_calibration(uint8_t target) {
  ...
  /* Start calibration... */
  Wire.beginTransmission(target);
  Wire.write(CALIBRATION_COMMAND_MSB);
  Wire.write(CALIBRATION_PREFIX);
  Wire.write(CALIBRATION_RESTORE_FACTORY);
...
void setup()
{
  ...
  /* Button pin, used to start background calibration */
...
#if (RESTORE_FACTORY_CALIBRATION == 1)  
  /* Reset calibration */
  reset_calibration(SUNRISE_ADDR);
#endif
  delay(readPeriodMs);
}
...
void read_sensor_measurements(uint8_t target) {
  ...
  /* Request values */
    error = WireRequestFrom(target, numBytes /* how many bytes */, ERROR_STATUS_LSB /* from address*/, true /* STOP*/);    
  if(error != numBytes ) {  
 ...
 * 
 * @param  target: The sensor's communication address
 * @note   This example shows a simple way to make background calibration.
 * @retval Error status
 */
int background_calibration(uint8_t target) {
  /* Function variables */
  int error;
  ...
  */
  ...
  if(error == 0) {
    ...
    Wire.write(CALIBRATION_COMMAND_MSB);
    Wire.write(CALIBRATION_PREFIX);
    Wire.write(CALIBRATION_BACKGROUND);
    error = Wire.endTransmission(true);
    ...
    if(error == 0) {
      ...
      do {
        ...
        */       
        ...
        error = WireRequestFrom(target, 1 /* how many bytes */, ERROR_STATUS_LSB /* from address*/, true /* STOP*/);  
        if(error == 1) {
          /* Error status */
          uint8_t eStatus = Wire.read();
          if(eStatus & ERROR_STATUS_LSB_CALIBRATION) {
            /* Calibration error */
            error = SLAVE_FAILURE;
          } else {
            /* Wakeup */
            ...
            if((error = WireRequestFrom(target, 1 /* how many bytes */, CALIBRATION_STATUS /* from address*/, true /* STOP*/)) == 1) {
              /* Calibration status */
              uint8_t statusCalibration = Wire.read();
              /* Clear error status, for return */
              error = 0u;
              /* Is calibration completed? */           
              if(statusCalibration & CALIBRATION_STATUS_BACKGROUND_COMPLETE) {
                break;
              } 
...
void loop() {
  ...
  /* Attempt to make background calibration if button pressed */
  if(digitalRead(BUTTON_PIN) == 0) {
     Serial.println("Trying to attempt background calibration process...");
     int error = background_calibration(SUNRISE_ADDR);
     if(0 == error) {
