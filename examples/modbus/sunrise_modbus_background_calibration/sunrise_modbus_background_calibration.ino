 ...
 *              
 * @author      William Sandkvist, Senseair FW team
 * @version     0.01
 * @date        2019-08-21
 *
 *******************************************************************************
...
/* Sensors registers */
const uint16_t HR1 = 0u;
const uint16_t HR2 = 1u;

/* Calibration statuses fro HR1 register */
const uint16_t HR1_RESET_VALUE                  = 0x0000u;
const uint16_t HR1_BACKGROUND_CALIBRATION       = 0x0020u;

/* Calibration command for HR2 register */
const uint16_t HR2_RESTORE_FACTORY_CALIBRATION  = 0x7C02u;
const uint16_t HR2_BACKGROUND_CALIBRATION       = 0x7C06u;


/* Error statuses */
const uint16_t IR1_FATAL_ERROR          = 0x0001u;
const uint16_t IR1_I2C_ERROR            = 0x0002u;
const uint16_t IR1_ALGORITHM_ERROR      = 0x0004u;
const uint16_t IR1_CALIBRATION_ERROR    = 0x0008u;
const uint16_t IR1_SELFDIAG_ERROR       = 0x0010u;
const uint16_t IR1_OUT_OF_RANGE_ERROR   = 0x0020u;
const uint16_t IR1_MEMORY_ERROR         = 0x0040u;
const uint16_t IR1_NO_MEASUREMENT_ERROR = 0x0080u;
const uint16_t IR1_NO_ERROR             = 0x0000u;
...
void setup() {
  ...
  /* Change measurement mode if single, 
     this example requered to use continuos measurement mode.
  */
...
int background_calibration(uint8_t target) {
  /* Function variables */
  static const uint16_t hr1_reset_value[] = { HR1_RESET_VALUE, };
  static const uint16_t hr2_background_cal[] = { HR2_BACKGROUND_CALIBRATION, };
  int error;
  ...
  */
  ...
  if(error = write_multiple_registers(SUNRISE_ADDR, HR1, 1, hr1_reset_value) == 0) {
    /* Start calibration... */
    if(error = write_multiple_registers(SUNRISE_ADDR, HR2, 1, hr2_background_cal) == 0) {
      int attempts = 0;
      do {
        ...
        */
        ...
        if((error = read_input_registers(target, ERROR_STATUS, 1)) == 0) {
          if(values[0] & IR1_CALIBRATION_ERROR) {
            /* Calibration error */
            error = SLAVE_FAILURE;
          }
          /* Second, check Calibration status */
          else if((error = read_holding_registers(target, HR1, 1)) == 0) {
...
void loop() {
  ...
  error_status = read_sensor_measurements(SUNRISE_ADDR);
  if(digitalRead(BUTTON_PIN) == 0) {
    /* If any error set, we cannot process calibration */
    if(IR1_NO_ERROR == error_status) {    
      ...
      Serial.println("Calibration process skiped due to exist errors...");
    }
  }
